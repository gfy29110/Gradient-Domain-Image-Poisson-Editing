...
void SquarePieces::ImageReigstInnerVetrtice(QImage * I_, MyPolygon mypolygon_)
{
	I = I_;
...
void SquarePieces::ImageReigstInnerVetrtice(QImage * I_, MyRectangle myrectangle_)
{
	I = I_;
	ScanningLine* scanningline = new ScanningLine;
...
void SquarePieces::ImageRegistforTest(QImage *I_, QImage *K_)
{
...
void SquarePieces::ImageRegistBoundary(QImage * J_, int Jx, int Jy)
{
	clock_t start_t, end_t;
	start_t = clock();
	J = J_;
...
void SquarePieces::ParameterSolve()
{
	UpdateParameter();
}

void SquarePieces::Reconstruction()
{
	clock_t start_t, end_t;
	start_t = clock();
	switch (RT)
...
void SquarePieces::SaveData()
{
	ofstream data("SquarePiecesData.txt");

	GetEnergy();
	data << "SquarePiecesData:" << endl;
...
void SquarePieces::RegistSquarePiece(MyMesh::FaceHandle & f_handle, MARK p00, MARK p01, MARK p11, MARK p10, bool token)
{
	Vec3f P, Q;
...
void SquarePieces::RegistSquarePiece_J(MyMesh::FaceIter & f_it, bool token)
{
	Vec3f P, Q, U_t;
	U_t = Vec3f(0, 0, 0);
...
void SquarePieces::UpdateXOnePiece(MyMesh::FaceIter & f_it)
{
...
void SquarePieces::UpdateZOnePiece(MyMesh::FaceIter & f_it)
{
	float x_weight, y_weight;
	...
	OpenMesh::Vec3f T_r, WXr, T_u, WXu;
	...
	while (fh_it != mesh->fh_end(*f_it)) {
		...
		if (mesh->data(*fh_it).getLocation() == U_Location) {
			T_u = mesh->data(*fh_it).getT();
			WXu = mesh->data(*fh_it).getX() + mesh->data(*fh_it).getU();
			x_weight = mesh->data(*fh_it).getW() ? eta : 1.0;
			u_fh_it = fh_it;
		}
		if (mesh->data(*fh_it).getLocation() == R_Location) {
			T_r = mesh->data(*fh_it).getT();
...
float SquarePieces::UpdateUOnePiece(MyMesh::FaceIter & f_it)
{
	float residual = 0.0;
...
void SquarePieces::PassingXOnePiece(MyMesh::FaceIter & f_it)
{
	MyMesh::FaceHalfedgeIter fh_it = mesh->fh_begin(*f_it);
	while (fh_it != mesh->fh_end(*f_it)) {
		if (mesh->data(*fh_it).getLocation() == R_Location || mesh->data(*fh_it).getLocation() == U_Location) {
			...
			if (!mesh->is_boundary(opposite_heh)) {
				...
				OpenMesh::Vec3f nX = (rho*(mesh->data(*fh_it).getZ() - mesh->data(*fh_it).getU()) + eta * mesh->data(*fh_it).getT()) / (rho + eta);
...
void SquarePieces::PassingZOnePiece(MyMesh::FaceIter & f_it)
{
	MyMesh::FaceHalfedgeIter fh_it = mesh->fh_begin(*f_it);
	while (fh_it != mesh->fh_end(*f_it)) {
...
float SquarePieces::UpdateOneStep()
{
	float residual = 0.0;
	UpdateX();
	UpdateZ();
...
void SquarePieces::UpdateParameter()
{
	float rho_max = 1e5;
	int i = 0;
	clock_t start_t, end_t;
...
void SquarePieces::ReconstructSeed(MyMesh::VertexHandle & v_it)
{
...
void SquarePieces::ReconstructBack()
{
...
float SquarePieces::EnergyFuntion()
{
	float energy = 0.0;
...
void SquarePieces::ReconstructDFS()
{
	clock_t start_t, end_t;
	start_t = clock();
...
void SquarePieces::ReconstructBFS()
{
	clock_t start_t, end_t;
	start_t = clock();
...
void SquarePieces::ReconstructFFT()
{
	double lambda = 1e7;
	double mu = 1e-5;
...
float SquarePieces::BoundaryResidual()
{
	float residual = 0.0;
...
Vec3f SquarePieces::color2vector(QRgb color)
{
...
void SquarePieces::CalculateCurl(Vec3f & curl, MyMesh::HalfedgeHandle hf)
{
...
float SquarePieces::EnergyofOnePiece(MyMesh::FaceIter f_it)
{
	float energy_1 = 0.0;
	float energy_2 = 0.0;
	...
	MyMesh::FaceHalfedgeIter fh_it = mesh->fh_begin(f_it);
	energy_1 += (mesh->data(*fh_it).getW() ? eta : 1.0)*(mesh->data(*fh_it).getT() - mesh->data(*fh_it).getX()).sqrnorm();
...
void SquarePieces::RecnstructOneEdge(MyMesh::VertexHandle & v_it, MyMesh::VertexHandle & u_it)
{
...
void SquarePieces::AdjustofHalfedge()
{
...
void SquarePieces::AdjustBoundaryData()
{
	for (auto he_it = mesh->halfedges_begin(); he_it != mesh->halfedges_end(); he_it++) {
		if (mesh->is_boundary(*he_it)) {
			MyMesh::HalfedgeHandle opposite_heh = mesh->opposite_halfedge_handle(*he_it);
...
gMARK SquarePieces::information4face(const MyMesh::FaceIter & f_it)
{
	gMARK token;
...
void SquarePieces::RegistMesh(bool token)
{
	clock_t start_t, end_t;
	//add vertex in mesh
	start_t = clock();
...
Vec3f SquarePieces::CalculateDUt(MyMesh::HalfedgeHandle hf)
{
	Vec3f curl;
	switch (mesh->data(hf).getLocation()) {
	case L_Location:
